...
public class BindingUtils {
    ...
    public static String getSourceAndTime(String sourceName, Timestamp date) {
        if (date==null){
            long  date_1 = System.currentTimeMillis()/1000;
            return sourceName + " • " + getElapsedTime(date_1);
        }
        return sourceName + " • " + getElapsedTime(date.getTime());
    }

    public static String getTime(String _dateFormat) {
        String finalDateTime="";
        Locale us = new Locale("US");
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'", us);
        try {
            Date date = format.parse(_dateFormat);
            ...
            finalDateTime = stringDate.concat(" ").concat(stringTime);
  ...
  /*  public static String getSourceAndDateTime(String sourceName, DateTime date) {
        return sourceName + " • " + getElapsedTime(date.getValue());
    }*/
    ...
    public static void loadImage(ImageView imageView, String url, String articleUrl) {
        ...
        Glide.with(imageView)
                .load(url)
                .apply(NewsGlideModule.roundedCornerImage(new RequestOptions(), imageView.getContext(), 5))
                .override(800,300)
                .placeholder(context.getResources().getDrawable(R.color.cardBackground))
